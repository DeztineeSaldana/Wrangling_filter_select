...
After running the code, I found out that there are 8 batman movies in the data. 
...
I believe that the second code is wrong because it is in the wrong order.
...
After running the codes, I found out that the longest movie was The Cure for Insomnia (5220 minutes). While the oldest movie was the Blacksmith Scene (1893).
...
After running the above code, I figured out that there are 1,248 movies made in 1995.

2. Which Comedies with an MPAA rating, over 120 minutes have a rating above 8?

```{r}
movies %>%
  filter(Comedy == 1, mpaa>0,  length > 120,rating > 8)
```

After running the above code, I found out there are 4 movies that fall under those categories.
...
After running the code above, I found out that there are 3 movies of Harry Potter included (The Sorcerer's Stone, The Chamber of Secrets, and The Prisoner of Azkaban). The movie with the highest rating was The Prisoner of Azkaban with a rating of 7.8, while the movie with the highest votes was Sorcerer's Stone with 46741 votes.
...
After running the code above, I found out that the movie with the highest budget for a movie that is an Animation or Comedy with at least 100 votes was Wild Wild West with a budget of $170000000.It got 19078 votes. 
